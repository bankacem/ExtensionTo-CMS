...
export default {
    ...
    async scheduled(event: any, env: Bindings, ctx: any) {
        const dbs = [env.DB_1, env.DB_2, env.DB_3];
        const query = "UPDATE articles SET status = 'published' WHERE status = 'scheduled' AND published_at <= CURRENT_TIMESTAMP";
        await Promise.all(dbs.map(db => db.prepare(query).run()));
        console.log("CRON: Ran scheduled post publisher across all shards.");
    }
};